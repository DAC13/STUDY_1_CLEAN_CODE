...
- 준혁: 광현이의 말처럼 테스트 코드를 작성함으로써 안전한 영역을 넓히며 진행하는 습관을 들여야겠다.
- 
### 10장 - 클래스

- SRP가 뭔가?
  - 승현: SRP는 하나의 모듈은 하나의 액터에 대해서만 책임져야 한다는 원칙
...
- 준혁: 준비 과정과 런타임 로직을 분리해야한다라는 내용이 중요하다고 느꼈다. 하지만, 후반부에 있는 AOP에 관한 이야기가 나온 경위는 잘 모르겠다.
...
- 승현: 현업에서 동시성 컨트롤을 하고 있다. 만석이 되면 더이상 예약이 불가능해야하고, 예약같은 경우엔 동일 좌석에 대한 예약을 제한한다. 한 버스에 대한 마감이 10초 정도만에 된다. DB level에서 row 단위로 lock을 걸어서 해결을 한다.
- 광현: 엄밀히 따지면 동시성을 제한하는 케이스인 것 같다. 결국 의도하지 않은 동시성을 미연에 방지하기 위해 제한하는 것도 동시성을 이해하는 길 중 하나라고 생각한다.
- 준혁: 자바는 기본적으로 멀티 스레드 기반인데 정작 멀티 스레드 방식에 대한 생각을 해본 적이 없어서 아쉽다. 스레드를 제한하는 방식도 때에 따라서는 필요하지만 멀티 방식에 대해서도 충분히 알아봐야겠다.
...
- @whdkwhdk90: 그 전에는 단순히 읽고 이해했던 것에 비해서 요번 내용들은 조금 더 깊고 사용 이유에 대해서 탐구해보는 시간이 되었다. 혼자 조금 더 골똘히 생각해보고 하는 시간을 가져야 할 것 같다.
- @SeungHyunP: 테스트 코드의 중요성을 다시 한번 느꼈다. SOLID원칙은 이미 공부했음에도 설명하는데 많이 버벅였던거 같다. 우리가 공부를 하는게 이해하는 것도 있지만 이해한바를 잘 설명하는 것도 중요하다고 생각하여, 다시 읽고 이해하며 설명하는 연습도 해야겠다.
## 다음 일정
